...
export const postFavouriteItemComment = {
    ...
    POST_FAVOURITE_ITEM_COMMENT_SUCCESS(state: State, action: PayloadAction<T.PostItemCommentSuccessPayload>) {
        const itemData = getItemDataById(state, action.payload.item_id) || {} as DeserializedItemData
        const commentsData = getItemCommentsDataById(state, action.payload.item_id) || {} as DeserializedListData<DeserializedCommentData>
        const parentId = action.payload.parent_id

        if (parentId) {
            const parentComment = getItemCommentDataById(state, action.payload.item_id, parentId) || {} as DeserializedCommentData
