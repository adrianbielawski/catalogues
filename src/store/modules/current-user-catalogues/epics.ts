...
import { concat, of, defer, forkJoin, Observable, merge, iif } from 'rxjs'
import { catchError, mergeMap, pluck, switchMap, withLatestFrom, filter, map, defaultIfEmpty } from 'rxjs/operators'
    ...
    switchMap(([_, id]) => concat(
        ...
        axiosInstance$.get('/catalogues/', {
            ...
            mergeMap(response => concat(
                ...
                of(actions.FETCH_CURRENT_USER_CATALOGUES_SUCCESS(response.data)),
            )),
            catchError(() => of(actions.FETCH_CURRENT_USER_CATALOGUES_FAILURE()))
        )
    ))
)
...
export const fetchCurrentUserCatalogueFieldsEpic = (
    ...
    mergeMap(action => concat(
        ...
        defer(() => axiosInstance$.get('/fields/', {
            ...
            mergeMap(([response, state]) => concat(
                ...
                of(actions.FETCH_CURRENT_USER_CATALOGUE_FIELDS_SUCCESS({
                    data: response.data,
                    catalogueId: action.payload
                })),
            )),
            catchError(() =>
                of(actions.FETCH_CURRENT_USER_CATALOGUE_FIELDS_FAILURE(action.payload))
            )
        )
    ))
)

export const fetchCurrentUserFieldsChoicesEpic = (action$: Observable<Action>) => merge(
    action$.pipe(filter(actions.FETCH_CURRENT_USER_CATALOGUE_FIELDS_SUCCESS.match)),
    ...
    return concat(
        of(actions.FETCH_CURRENT_USER_FIELDS_CHOICES_START(action.payload.catalogueId)),
        forkJoin<typeof requests, string>(requests).pipe(
            ...
            mergeMap(data => concat(
                ...
                of(actions.FETCH_CURRENT_USER_FIELDS_CHOICES_SUCCESS({
                    catalogueId: action.payload.catalogueId,
                    data,
                })),
            )),
            catchError(() =>
                of(actions.FETCH_CURRENT_USER_FIELDS_CHOICES_FAILURE(action.payload.catalogueId))
            )
        )
    )
}))
