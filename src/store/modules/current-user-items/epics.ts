...
export const refreshItemEpic = (action$: Observable<Action>) =>
  ...
  ).pipe(map((action) => actions.FETCH_CURRENT_USER_ITEM(action.payload)))

export const fetchItemEpic = (action$: Observable<Action>) =>
  action$.pipe(
        ...
        defer(() => axiosInstance$.get(`/items/${action.payload}/`)).pipe(
          ...
          mergeMap((response) =>
            concat(
              ...
              of(
                itemsActions.ITEM_UPDATED({
                  id: response.data.id,
                  changes: itemDeserializer(response.data),
                }),
              ),
              of(
                actions.FETCH_CURRENT_USER_ITEM_SUCCESS({
                  data: response.data,
          ...
          catchError(() =>
            of(actions.FETCH_CURRENT_USER_ITEM_FAILURE(action.payload)),
          ),
        ),
      ),
    ),
...
export const fetchItemsEpic = (action$: Observable<Action>) =>
  action$.pipe(
            ...
            concat(
              of(
                usersActions.USERS_ADDED(
                  response.data.results.map((i: Item) => i.created_by),
                ),
              ),
              of(itemsActions.ITEMS_UPDATED(response.data.results)),
              of(
                actions.FETCH_CURRENT_USER_ITEMS_SUCCESS({
                  data: response.data,
...
export const addItemEpic = (action$: Observable<Action>) =>
  action$.pipe(
        ...
        request$.pipe(
          mergeMap((response) =>
            imagesRequests$(response.data.id).pipe(
              ...
              catchError(() =>
                of(actions.SAVE_ITEM_FAILURE(action.payload.id)),
              ),
            ),
          ),
...
export const deleteItemEpic = (action$: Observable<Action>) =>
  action$.pipe(
...
export const changeItemRatingEpic = (action$: Observable<Action>) =>
  action$.pipe(
...
export const changeFavouriteItemEpic = (action$: Observable<Action>) =>
  action$.pipe(
...
export const fetchItemsCommentsEpic = (action$: Observable<Action>) =>
  action$.pipe(
...
export const fetchItemCommentsEpic = (action$: Observable<Action>) =>
  action$.pipe(
          ...
          .pipe(
            mergeMap((response) =>
              concat(
                ...
                of(
                  actions.FETCH_ITEM_COMMENTS_SUCCESS({
            ...
            catchError(() =>
              of(actions.FETCH_ITEM_COMMENTS_FAILURE(action.payload.itemId)),
            ),
          ),
      ),
    ),
...
export const postItemCommentEpic = (action$: Observable<Action>) =>
  action$.pipe(
        ...
        defer(() =>
          axiosInstance$
            ...
            .pipe(
              ...
              catchError(() =>
                of(actions.POST_ITEM_COMMENT_FAILURE(action.payload.itemId)),
              ),
            ),
        ),
      ),
