...
export const fetchTopItemsEpic = (action$: Observable<Action>) => action$.pipe(
    filter(actions.FETCH_TOP_ITEMS.match),
    switchMap((action) => concat(
        of(actions.FETCH_TOP_ITEMS_START()),
        axiosInstance$.get('/items/top/', {
            params: {
                page: action.payload,
            }
        }).pipe(
...
export const fetchTopItemsChoicesEpic = (action$: Observable<Action>) => action$.pipe(
...
export const postTopItemCommentEpic = (action$: Observable<Action>) => action$.pipe(
    ...
    mergeMap(action => concat(
        ...
        defer(() => axiosInstance$.post(`/comments/`, {
            ...
            catchError(() => of(actions.POST_TOP_ITEM_COMMENT_FAILURE(action.payload.itemId)))
        ))
    ))
)

export const fetchTopItemCommentsEpic = (action$: Observable<Action>) => action$.pipe(
    ...
    mergeMap(action => concat(
        ...
        axiosInstance$.get(`/comments/`, {
            ...
            mergeMap(response => concat(
                ...
                of(actions.FETCH_TOP_ITEM_COMMENTS_SUCCESS({
                    itemId: action.payload.itemId,
                    data: response.data
                }))
            )),
            catchError(() => of(actions.FETCH_TOP_ITEM_COMMENTS_FAILURE(action.payload.itemId)))
        )
    ))
)
