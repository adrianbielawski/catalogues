...
export const fetchTopItemsEpic = (action$: Observable<Action>) =>
  action$.pipe(
    filter(actions.FETCH_TOP_ITEMS.match),
    switchMap((action) =>
      concat(
        of(actions.FETCH_TOP_ITEMS_START()),
        axiosInstance$
          .get('/items/top/', {
            params: {
              page: action.payload,
          ...
          .pipe(
            mergeMap((response) => {
              const users = (response.data.results as Item[]).map(
...
export const fetchTopItemsCataloguesEpic = (
  ...
  action$.pipe(
    ...
    switchMap(([action, ids]) => {
      ...
      if (filteredIds.length === 0) {
        return of(actions.FETCH_TOP_ITEMS_CATALOGUES_SUCCESS(allIds))
      }
      ...
      return forkJoin(requests).pipe(
        mergeMap((response) =>
          concat(
            ...
            of(actions.FETCH_TOP_ITEMS_CATALOGUES_SUCCESS(allIds)),
          ),
        ),
        catchError(() => of(actions.FETCH_TOP_ITEMS_CATALOGUES_FAILURE())),
      )
    }),
  )

export const fetchTopItemsCommentsEpic = (action$: Observable<Action>) =>
  action$.pipe(
    filter(actions.FETCH_TOP_ITEMS_SUCCESS.match),
    mergeMap((action) => {
...
export const fetchTopItemsFieldsEpic = (
  action$: Observable<Action>,
  state$: Observable<RootState>,
) =>
  action$.pipe(
    filter(actions.FETCH_TOP_ITEMS_CATALOGUES_SUCCESS.match),
    ...
    mergeMap(([action, fields]) => {
      ...
      if (filteredIds.length === 0) {
        return of(actions.TOP_ITEMS_FIELDS_NOT_NEEDED())
      }
      ...
      return forkJoin(requests).pipe(
        ...
        mergeMap((response) =>
          concat(
            ...
            of(actions.FETCH_TOP_ITEMS_FIELDS_SUCCESS(response)),
          ),
        ),
        catchError(() => of(actions.FETCH_TOP_ITEMS_FIELDS_FAILURE())),
      )
    }),
  )

export const fetchTopItemsChoicesEpic = (action$: Observable<Action>) =>
  action$.pipe(
    filter(actions.FETCH_TOP_ITEMS_FIELDS_SUCCESS.match),
    mergeMap((action) => {
        ...
        forkJoin(requests).pipe(
          ...
          mergeMap((data) => {
            ...
            return concat(
              ...
              of(actions.FETCH_TOP_ITEMS_CHOICES_SUCCESS(choices)),
            )
          }),
          catchError(() => of(actions.FETCH_TOP_ITEMS_CHOICES_FAILURE())),
        ),
      )
    }),
  )

export const postTopItemCommentEpic = (action$: Observable<Action>) =>
  action$.pipe(
    filter(actions.POST_TOP_ITEM_COMMENT.match),
    mergeMap((action) =>
      concat(
        of(actions.POST_TOP_ITEM_COMMENT_START(action.payload.itemId)),
        defer(() =>
          axiosInstance$
            .post('/comments/', {
            ...
            .pipe(
              mergeMap((response) =>
                concat(
                  ...
                  of(actions.POST_TOP_ITEM_COMMENT_SUCCESS(response.data)),
                ),
              ),
              catchError(() =>
                of(
                  actions.POST_TOP_ITEM_COMMENT_FAILURE(action.payload.itemId),
                ),
              ),
            ),
        ),
...
export const fetchTopItemCommentsEpic = (action$: Observable<Action>) =>
  action$.pipe(
    filter(actions.FETCH_TOP_ITEM_COMMENTS.match),
    mergeMap((action) =>
      concat(
        of(actions.FETCH_TOP_ITEM_COMMENTS_START(action.payload.itemId)),
        axiosInstance$
          .get('/comments/', {
            params: {
              item_id: action.payload.itemId,
          ...
          .pipe(
            mergeMap((response) =>
              concat(
                ...
                of(
                  actions.FETCH_TOP_ITEM_COMMENTS_SUCCESS({
            ...
            catchError(() =>
              of(
                actions.FETCH_TOP_ITEM_COMMENTS_FAILURE(action.payload.itemId),
              ),
            ),
          ),
      ),
