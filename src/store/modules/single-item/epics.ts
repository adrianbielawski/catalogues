...
import { DeserializedImage, ItemCommentParent } from "src/globalTypes"
//Serializers
...
export const fetchSingleItemEpic = (action$: Observable<Action>) => action$.pipe(
    ...
    switchMap((action) => concat(
        ...
        axiosInstance$.get(`/items/${action.payload}/`).pipe(
            ...
            catchError(error => of(actions.FETCH_SINGLE_ITEM_FAILURE(error.response.status)))
        )
    ))
)

export const refreshSingleItemCommentsEpic = (action$: Observable<Action>) => merge(
    action$.pipe(filter(actions.FETCH_MORE_SINGLE_ITEM_COMMENTS.match)).pipe(map(action =>({
            itemId: action.payload.itemId,
            page: action.payload.page,
        })
    )),
    action$.pipe(filter(actions.FETCH_SINGLE_ITEM_SUCCESS.match)).pipe(map(action =>({
            itemId: action.payload.id,
            page: 1,
        })
    )),
).pipe(
    map(payload => actions.FETCH_SINGLE_ITEM_COMMENTS(payload))
)

export const fetchSingleItemCommentsEpic = (action$: Observable<Action>) => action$.pipe(
...
export const fetchSingleItemChoicesEpic = (action$: Observable<Action>) => action$.pipe(
...
export const postSingleItemCommentEpic = (action$: Observable<Action>) => action$.pipe(
    ...
    mergeMap(action => concat(
        ...
        defer(() => axiosInstance$.post(`/comments/`, {
            ...
            catchError(() => of(actions.POST_SINGLE_ITEM_COMMENT_FAILURE(action.payload.itemId)))
        ))
    ))
)
        ...
        return concat(
            ...
            request$.pipe(
                mergeMap(response => imagesRequests$(response.data.id).pipe(
                    ...
                    catchError(() => of(actions.SAVE_SINGLE_ITEM_FAILURE(action.payload.id)))
                ))
            )
        )
    })
...
export const refreshSingleItemEpic = (action$: Observable<Action>) => merge(
    action$.pipe(filter(actions.REFRESH_SINGLE_ITEM.match)),
    action$.pipe(filter(actions.SAVE_SINGLE_ITEM_SUCCESS.match)),
).pipe(
    map(action => actions.FETCH_SINGLE_ITEM(action.payload))
)
