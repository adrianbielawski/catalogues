...
export const fetchLatestItemsEpic = (action$: Observable<Action>) => action$.pipe(
...
export const fetchLatestItemsCommentsEpic = (action$: Observable<Action>) => action$.pipe(
    filter(actions.FETCH_LATEST_ITEMS_SUCCESS.match),
    mergeMap(action => {
...
export const fetchLatestItemsChoicesEpic = (action$: Observable<Action>) => action$.pipe(
...
export const postLatestItemCommentEpic = (action$: Observable<Action>) => action$.pipe(
    ...
    mergeMap(action => concat(
        ...
        defer(() => axiosInstance$.post(`/comments/`, {
            ...
            catchError(() => of(actions.POST_LATEST_ITEM_COMMENT_FAILURE(action.payload.itemId)))
        ))
    ))
)

export const fetchLatestItemCommentsEpic = (action$: Observable<Action>) => action$.pipe(
    ...
    mergeMap(action => concat(
        ...
        axiosInstance$.get(`/comments/`, {
            ...
            mergeMap(response => concat(
                ...
                of(actions.FETCH_LATEST_ITEM_COMMENTS_SUCCESS({
                    itemId: action.payload.itemId,
                    data: response.data
                }))
            )),
            catchError(() => of(actions.FETCH_LATEST_ITEM_COMMENTS_FAILURE(action.payload.itemId)))
        )
    ))
)
