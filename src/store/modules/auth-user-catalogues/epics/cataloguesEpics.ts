...
import { catchError, mergeMap, pluck, switchMap, withLatestFrom, filter, map } from 'rxjs/operators'
//Types
...
export const refreshCatalogueEpic = (action$: Observable<Action>) => action$.pipe(
    ...
    switchMap(action => concat(
        axiosInstance$.get(`/catalogues/${action.payload}/`).pipe(
            map(response =>
                of(cataloguesActions.CATALOGUE_UPDATED({
                    id: action.payload,
                    changes: {
            ...
            catchError(() => of(actions.REFRESH_CATALOGUE_FAILURE(action.payload)))
        )
    ))
)
    ...
    switchMap(action => concat(
        ...
        axiosInstance$.patch(`/catalogues/${action.payload.catalogueId}/`, {
            ...
            mergeMap(response => concat(
                ...
                of(actions.CHANGE_CATALOGUE_NAME_SUCCESS(action.payload.catalogueId)),
            )),
    ...
    switchMap(action => concat(
        ...
        axiosInstance$.patch(`/catalogues/${action.payload.catalogueId}/`, {
            ...
            catchError(() => concat(
                ...
                of(actions.CHANGE_DEFAULT_CATALOGUE_FAILURE(
    ...
    switchMap(action => concat(
        ...
        axiosInstance$.patch(`/catalogues/${action.payload.catalogueId}/`, {
            ...
            catchError(() => concat(
                ...
                of(actions.CHANGE_PUBLIC_CATALOGUE_FAILURE(
                    action.payload.catalogueId,
                ))
            ))
        )
    ...
    switchMap(action => concat(
        ...
        from(fetch(action.payload.image)).pipe(
            ...
            mergeMap(response => concat(
                ...
                of(actions.POST_CATALOGUE_IMAGE_SUCCESS(action.payload.catalogueId)),
            )),
...
export const addCatalogueToFavouritesEpic = (action$: Observable<Action>) => action$.pipe(
    ...
    switchMap(action => concat(
        ...
        defer(() => axiosInstance$.put(`/catalogues/${action.payload}/favourite/`, {
            is_favourite: true,
        }).pipe(
...
export const deleteCatalogueFromFavouriteEpic = (action$: Observable<Action>) => action$.pipe(
    ...
    switchMap(action => concat(
        ...
        defer(() => axiosInstance$.delete(`/catalogues/${action.payload}/favourite/`).pipe(
            map(() => FETCH_FAVOURITE_CATALOGUES()),
            catchError(() => concat(
                of(actions.DELETE_CATALOGUE_FROM_FAVOURITE_FAILURE(action.payload)),
                of(cataloguesActions.CATALOGUE_UPDATED({
                    id: action.payload,
                    changes: {
                        isFavourite: true,
