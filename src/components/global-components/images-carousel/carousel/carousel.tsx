...
const Carousel = (props: Props) => {
  ...
  const [current, setCurrent] = useState(
    props.images.findIndex((img) => img.isPrimary),
  )
  ...
  useEffect(() => {
    if (prevIds != null && props.images.length > prevIds.length) {
      const newImage = props.images.filter(
        (image) => !prevIds?.includes(image.id),
      )[0]
  ...
  const getDynamicStyles = (i: number) => {
    ...
    if (swipeX) {
      const scaleValue = Math.abs((swipeX % 1) * 0.3)

      let scale =
        i === current
          ? clamp(MAX_SCALE - scaleValue, MIN_SCALE, MAX_SCALE)
          : MIN_SCALE

      if (
        (swipeX < 0 && i === current + 1) ||
        (swipeX > 0 && i === current - 1)
      ) {
        scale = clamp(MIN_SCALE + scaleValue, MIN_SCALE, MAX_SCALE)
      }
      ...
      items.push(
        <li key={i} ref={imgRef}>
          <CarouselItem
            ...
            onRemove={props.onRemove != null ? onRemove : undefined}
  ...
  const carouselClass = cx('carousel', {
    singleView: props.singleView,
    withShadow: props.withShadow,
  })
