...
const MediaFieldValue = (props: Props) => {
  ...
  const internalPlayer = props.fieldValue.service
    ? servicesMap[props.fieldValue.service]?.internalPlayer?.includes(
  ...
  const placeholder = <FontAwesomeIcon className={styles.icon} icon={faLink} />

  return (
    <div className={styles.mediaField} onClick={handleMediaFieldClick}>
      <Image className={styles.image} url={image} placeHolder={placeholder} />
      <p className={styles.title}>
        {props.fieldValue.title ? props.fieldValue.title : props.fieldValue.url}
      </p>
      {internalPlayer && (
        <AnimatedModal
          show={showPlayer}
